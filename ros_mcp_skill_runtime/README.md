...
Start with the mock backend first. It does not require ROS, turtlesim, rosbridge, or a physical robot, so it is the safest way to verify the runtime behavior.
...
If you are using the repository-level virtual environment from this checkout, use:

```bash
cd ros_mcp_skill_runtime
PYTHONPATH=src:/workspace/ros-mcp-server/.venv/lib/python3.10/site-packages python -m pytest
```

Expected result: all tests pass. These tests cover manifest loading, duplicate/invalid manifest rejection, permission rejection, safety-tier rejection, input validation, mock execution, JSONL log writing, and `stop_robot()` behavior.
...
- `list_skills` includes `move_to_joint_positions`, `move_arm_home`, gripper skills, `get_robot_state`, and `stop_robot`.
...
Expected result: the MCP client only sees skill-level robot tools, validation rejects the bad joint vector before execution, and `logs/skill_events.jsonl` contains one JSON record per dry-run/execution.
